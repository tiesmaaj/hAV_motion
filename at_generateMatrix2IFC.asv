function [data_output] = at_generateMatrix2IFC(catchtrials, congruent_mstrials, incongruent_mstrials, stimtrials, audInfo, visInfo, right_var, left_var, catch_var, cued_nature)
...
r_cong_rA_incong = zeros(stimtrials(1,3) * length(audInfo.cohSet), 6);
r_cong_lA_incong = zeros(stimtrials(1,3) * length(audInfo.cohSet), 6);
l_cong_rA_incong = zeros(stimtrials(1,3) * length(audInfo.cohSet), 6);
l_cong_lA_incong = zeros(stimtrials(1,3) * length(audInfo.cohSet), 6);
rA_incong_r_cong = zeros(stimtrials(1,3) * length(audInfo.cohSet), 6);
rA_incong_l_cong = zeros(stimtrials(1,3) * length(audInfo.cohSet), 6);
lA_incong_r_cong = zeros(stimtrials(1,3) * length(audInfo.cohSet), 6);
lA_incong_l_cong = zeros(stimtrials(1,3) * length(audInfo.cohSet), 6);
...
% A_r_trials = zeros(stimtrials(1,1) * length(audInfo.cohSet), 4);
% A_l_trials = zeros(stimtrials(1,1) * length(audInfo.cohSet), 4);
% V_l_trials = zeros(stimtrials(1,2) * length(visInfo.cohSet), 4);
% V_r_trials = zeros(stimtrials(1,2) * length(visInfo.cohSet), 4);
...
%     A_r_trials((i-1)*stimtrials(1)+1:i*stimtrials(1), :) = [repmat([right_var, cohAud, NaN, NaN], stimtrials(1), 1)];
%     A_l_trials((i-1)*stimtrials(1)+1:i*stimtrials(1), :) = [repmat([left_var, cohAud, NaN, NaN], stimtrials(1), 1)];
%     
%     % Visual only trials
%     V_r_trials((i-1)*stimtrials(2)+1:i*stimtrials(2), :) = [repmat([NaN, NaN, right_var, cohVis], stimtrials(2), 1)];
%     V_l_trials((i-1)*stimtrials(2)+1:i*stimtrials(2), :) = [repmat([NaN, NaN, left_var, cohVis], stimtrials(2), 1)];
end
...
if stimtrials(1,1) == 0 && stimtrials(1,2) > 0
    data_output(:, [1 2]) = [];
end
