function [data_output] = at_generateMatrixSKEW(catchtrials, L_congruent_mstrials, R_congruent_mstrials, lA_incongruent_mstrials, rA_incongruent_mstrials, L_stimtrials, R_stimtrials, audInfo, visInfo, right_var, left_var, catch_var, cued_nature)
...
AV_r_cong_trials = zeros(R_congruent_mstrials * length(audInfo.cohSet), 4);
AV_l_cong_trials = zeros(L_congruent_mstrials * length(audInfo.cohSet), 4);
AV_rA_incong_trials = zeros(rA_incongruent_mstrials * length(audInfo.cohSet), 4);
AV_lA_incong_trials = zeros(lA_incongruent_mstrials * length(audInfo.cohSet), 4);
A_r_trials = zeros(R_stimtrials(1,1) * length(audInfo.cohSet), 4);
A_l_trials = zeros(L_stimtrials(1,1) * length(audInfo.cohSet), 4);
V_r_trials = zeros(R_stimtrials(1,2) * length(visInfo.cohSet), 4);
V_l_trials = zeros(L_stimtrials(1,2) * length(visInfo.cohSet), 4);
...
for i = 1:length(audInfo.cohSet)
    ...
    % Congruent trials
    AV_r_cong_trials((i-1)*R_congruent_mstrials+1:i*R_congruent_mstrials, :) = [repmat([right_var, cohAud, right_var, cohVis], R_congruent_mstrials, 1)];
    AV_l_cong_trials((i-1)*L_congruent_mstrials+1:i*L_congruent_mstrials, :) = [repmat([left_var, cohAud, left_var, cohVis], L_congruent_mstrials, 1)];

    % Incongruent trials
    AV_rA_incong_trials((i-1)*rA_incongruent_mstrials+1:i*rA_incongruent_mstrials, :) = [repmat([right_var, cohAud, left_var, cohVis], rA_incongruent_mstrials, 1)];
    AV_lA_incong_trials((i-1)*lA_incongruent_mstrials+1:i*lA_incongruent_mstrials, :) = [repmat([left_var, cohAud, right_var, cohVis], lA_incongruent_mstrials, 1)];

    % Auditory only trials
    A_r_trials((i-1)*R_stimtrials(1)+1:i*R_stimtrials(1), :) = [repmat([right_var, cohAud, NaN, NaN], R_stimtrials(1), 1)];
    A_l_trials((i-1)*L_stimtrials(1)+1:i*L_stimtrials(1), :) = [repmat([left_var, cohAud, NaN, NaN], L_stimtrials(1), 1)];
    
    % Visual only trials
    V_r_trials((i-1)*R_stimtrials(2)+1:i*R_stimtrials(2), :) = [repmat([NaN, NaN, right_var, cohVis], R_stimtrials(2), 1)];
    V_l_trials((i-1)*L_stimtrials(2)+1:i*L_stimtrials(2), :) = [repmat([NaN, NaN, left_var, cohVis], L_stimtrials(2), 1)];
end
...
if L_stimtrials(1,1) == 0 && L_stimtrials(1,2) > 0
    data_output(:, [1 2]) = [];
end
