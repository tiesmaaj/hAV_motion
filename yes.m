...
right_var = 1; left_var = 2; catch_var = 0; cong_var = 1; incong_var = 0;

%% Specify parameters of the block 
disp('This is the main script for the AV MOTION congruency discrimination task.')
%task_nature = input('Staircase = 1;  Method of constant stimuli (MCS) = 2 : ');
task_nature = 2;
vel_stair = 0;
stim_matching_nature = 3;
stim_or_perf_matched = 1;
interleave_nature = 0;
selfinit_nature = 0;
cued_nature = 0;
cued_typeBlock = 0;
...
congruent_keypress = [115 13 117 14]; % 1 on button box
incongruent_keypress = [114 12 116 15]; % 0 on button box
...
% New conditions:
num_trials = 100; stimtrials = [0, 0, 0]; catchtrials = 0;
congruent_mstrials = 20; incongruent_mstrials = 20;
...
elseif task_nature == 2
    if stim_matching_nature == 1
        ...
        if length(audInfo.cohSet) == length(visInfo.cohSet)
            ...
            if stim_vs_perf_match_nature
                ...
                [data_output_2above] = at_generateMatrixALL(0, congruent_mstrials, incongruent_mstrials, stimtrials, audInfo_2above, visInfo_2above, right_var, left_var, catch_var);
                [data_output_1above] = at_generateMatrixALL(0, congruent_mstrials, incongruent_mstrials, stimtrials, audInfo_1above, visInfo_1above, right_var, left_var, catch_var);
                [data_output_1below] = at_generateMatrixALL(0, congruent_mstrials, incongruent_mstrials, stimtrials, audInfo_1below, visInfo_1below, right_var, left_var, catch_var);
                [data_output_2below] = at_generateMatrixALL(0, congruent_mstrials, incongruent_mstrials, stimtrials, audInfo_2below, visInfo_2below, right_var, left_var, catch_var);

                all_trials = [data_output_PERF; data_output_2above; data_output_1above; data_output_1below; data_output_2below];
                ...
                if cued_nature
                    ...
                    nan_rows = any(isnan(all_trials(:, 1:4)), 2);
                    valid_rows = ~nan_rows;
                    
                    % Assign 0 to column 9 for rows with NaNs in columns 1-4
                    all_trials(nan_rows, 9) = 0;
                    ...
                    all_trials(valid_rows, 9) = nums_valid';
                    
                end

                if cued_nature
                    if all_noise_nature
                        all_trials = [all_trials; data_output_NOISE_AUD; data_output_NOISE_VIS];
                    end
                else
                    if all_noise_nature
                        all_trials = [all_trials; data_output_STIM_AUD; data_output_STIM_VIS; data_output_NOISE_AUD; data_output_NOISE_VIS];
                    end
                end
    ...
    elseif stim_matching_nature == 3
        ...
        all_trials = at_generateMatrix_2IFC(catchtrials, congruent_mstrials, incongruent_mstrials, stimtrials, audInfo, visInfo, right_var, left_var, catch_var, cued_nature);
...
instructions_congAV(curWindow, cWhite0);
...
%% trial generation
%% Trial Loop for Two-Interval Forced-Choice (2IFC) Task Design
% The task presents audiovisual motion in two intervals.
% The participant reports which interval had the matching auditory and visual motion going in the same direction.
...
for ii = 1:length(data_output)
    ...
    % Set direction for the first interval
    visInfo.dir = data_output(ii, 3);  % Direction of visual motion for the first interval
    audInfo.dir = data_output(ii, 1);  % Direction of auditory motion for the first interval

    dotInfo = at_createDotInfo(inputtype, visInfo.coh, visInfo.dir, typeInt, minNum, maxNum, meanNum, maxdotsframe, dur, visInfo.vel, visInfo.displaceSet);
    ...
    % Create and Present the First Interval Stimulus (Audiovisual)
    ...
    % Present the first interval stimulus
    [resp, rt, start_time] = at_presentAV(continue_show, responded, resp, rt, curWindow, fix, frames, pahandle, wavedata, visInfo, dotInfo, center, dotSize, d_ppd, ndots, dxdy, ss, Ls, EEG_nature, outlet, markers);

    Screen('DrawingFinished',curWindow);
    Screen('DrawDots', curWindow, [0; 0], 10, [255 0 0], fix, 1);
    Screen('Flip', curWindow,0);

    % Jitter the time between the first and second interval
    jitterTime = makeInterval(typeInt, 1.0, 2.0, 1.5);  % Generate jitter time
    WaitSecs(jitterTime);  % Add jittered wait time between intervals
    
    % Set direction for the second interval
    visInfo.dir = data_output(ii, 7);  % Direction of visual motion for the second interval
    audInfo.dir = data_output(ii, 5);  % Direction of auditory motion for the second interval

    dotInfo = at_createDotInfo(inputtype, visInfo.coh, visInfo.dir, typeInt, minNum, maxNum, meanNum, maxdotsframe, dur, visInfo.vel, visInfo.displaceSet);

    
    % Create and Present the Second Interval Stimulus (Audiovisual)
    [center, dotSize, d_ppd, ndots, dxdy, ss, Ls, continue_show] = at_generateDot(visInfo, dotInfo, screenInfo, screenRect, monWidth, viewDist, maxdotsframe, dur, curWindow, fix);
    frames = 0;
    CAM = makeCAM_PILOT(audInfo.coh, audInfo.dir, audInfo.durRaw, silence, Fs, noise_reduction_scalar, noise_jitter_nature);
    ...
    % Present the second interval stimulus
    [resp, rt, start_time] = at_presentAV(continue_show, responded, resp, rt, curWindow, fix, frames, pahandle, wavedata, visInfo, dotInfo, center, dotSize, d_ppd, ndots, dxdy, ss, Ls, EEG_nature, outlet, markers);
    ...
    %% Collect response from participant on which interval was congruent
    % Prompt for which interval had congruent motion (matching auditory and visual motion direction)
    DrawFormattedText(curWindow, 'Which interval had matching auditory and visual motion directions? Press 1 for First, 2 for Second', 'center', 'center', [255 255 255]);
    Screen('Flip', curWindow);
    
    % Wait for response (1 = first interval, 2 = second interval)
    [keyIsDown, ~, keyCode] = KbCheck;
    while ~keyIsDown
        [keyIsDown, ~, keyCode] = KbCheck;
    end
    if keyCode(congruent_keypress(1)) || keyCode(congruent_keypress(2))  % Response for First Interval
        resp = 1; % First Interval selected
    elseif keyCode(congruent_keypress(3)) || keyCode(congruent_keypress(4)) % Response for Second Interval
        resp = 2; % Second Interval selected
    end
    
    % Record response and reaction time (rt)
    rt = GetSecs - start_time; % Reaction time calculation
    data_output(ii, 8) = resp;  % Store response for the trial
    data_output(ii, 9) = rt;  % Store reaction time for the trial

    %% ITI (Inter-trial interval) Logic
