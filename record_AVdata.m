function [trial_status, data_output] = record_AVdata(data_output, cong_var, incong_var, congruent_keypress, incongruent_keypress, audInfo, visInfo, resp, rt, ii, vel_stair, interleave_nature, sliderResp)
...
if interleave_nature == 1
    if data_output(ii,1) == data_output(ii,3) || any(isnan(data_output(ii,1:4)))
    ...
    elseif data_output(ii,1) == 0 && data_output(ii,3) > 0
        if data_output(ii,3) == data_output(ii,5)
            trial_status = 1;
            data_output(ii, 8) = trial_status;
        else
            trial_status = 0;
    ...
    elseif data_output(ii,1) > 0 && data_output(ii,3) == 0
        if data_output(ii,1) == data_output(ii,5)
            trial_status = 1;
            data_output(ii, 8) = trial_status;
        else
            trial_status = 0;
