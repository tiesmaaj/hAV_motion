...
%% FOR RESPONSE CODING: 1 = UPWARD MOTION; 2 = DOWNWARD MOTION
...
ExampleMatrix = 0;
OS_nature = 1;
skewed_prior_nature = 0; % 0= none, 1= right, 2= left
...
right_keypress = [115 13 112];
left_keypress = [114 12 117];
...
num_trials = 250; stimtrials = [0, 20, 0]; catchtrials = 20;
...
elseif task_nature == 2
    ...
    elseif stim_matching_nature == 3
        audInfo.cohSet = [0.7, 0.4, 0.2, 0.10, 0.05];
        visInfo.cohSet = [0.7, 0.4, 0.2, 0.10, 0.05];
        %visInfo.cohSet = [1.0 1.0 1.0 1.0];

        if skewed_prior_nature
            L_stimtrials = [0, 5, 0];
            R_stimtrials = [0, 20, 0];
            all_trials = at_generateMatrixSKEW(catchtrials, 0, 0, 0, 0, L_stimtrials, R_stimtrials, audInfo, visInfo, right_var, left_var, catch_var, 0);
        else
...
instructions_vertVis(curWindow, cWhite0);
...
for ii = 1:length(data_output)
    ...
    visInfo = direction_conversion_VERT(visInfo);
