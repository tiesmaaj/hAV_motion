...
congruent_keypress = [115 13 14]; % 1 on button box
incongruent_keypress = [114 12 15]; % 0 on button box
...
instructions_congAV_PILOT(curWindow, cWhite0); % congAV and congAV_PILOT have same instructions currently
...
for ii = 1:length(data_output)
    ...
    if ~isnan(audInfo.dir) && ~isnan(audInfo.coh)
        ...
        % to another
        CAM = makeCAM_ANG(audInfo.coh, audInfo.dir, audInfo.durRaw, silence, Fs, noise_reduction_scalar, noise_jitter_nature);
   ...
   %% AV Stimuli Presentation
    ...
    [trial_status, data_output] = record_AVdata(data_output, cong_var, incong_var, congruent_keypress, incongruent_keypress, audInfo, visInfo, resp, rt, ii, vel_stair, interleave_nature, sliderResp);
