...
addpath(genpath('/home/wallace/Human_AV_Motion/liblsl-Matlab-master/'));
addpath(genpath('/home/wallace/Human_AV_Motion/liblsl-Matlab-master/bin/'));
...
%% New conditions for interleaved AO, VO, AV EEG task
% trials_per_condition = input('Number of trials per condition: ');
% congruent_mstrials = trials_per_condition; incongruent_mstrials = trials_per_condition;
% cohSet = input('Coherence(s) (separate by comma): ', 's');
% cohSet = str2double(cohSet);
% modality = input('AO (1), VO (2), or AV (3): ');

if isCong == 1 && isDir == 0
    trials_per_condition = 40; congruent_mstrials = trials_per_condition; incongruent_mstrials = trials_per_condition;
    audInfo.cohSet = [1.0, 0.2]; visInfo.cohSet = [1.0, 0.2];
    modality = 3; stimtrials = [0, 0, trials_per_condition];
    catchtrials = 0;
elseif isCong == 0 && isDir == 1
    trials_per_condition = 40; congruent_mstrials = trials_per_condition; incongruent_mstrials = trials_per_condition;
    audInfo.cohSet = [1.0, 0.2]; visInfo.cohSet = [1.0, 0.2];
    modality = 3; stimtrials = [trials_per_condition, trials_per_condition, trials_per_condition];
    catchtrials = 0;
end

% if modality == 1
%     num_trials = 250; stimtrials = [trials_per_condition, 0, 0]; catchtrials = 0;
% elseif modality == 2
%     num_trials = 250; stimtrials = [0, trials_per_condition, 0]; catchtrials = 0;
% elseif modality == 3 
%     %num_trials = 250; stimtrials = [0, 0, trials_per_condition]; catchtrials = 0;
%     num_trials = 250; stimtrials = [trials_per_condition, trials_per_condition, trials_per_condition]; catchtrials = 0;
% else
...
elseif task_nature == 2
            ...
            if stim_vs_perf_match_nature
                ...
                if cued_nature
                    if all_noise_nature
                        ...
                        nums = nums(randperm(right_varlength(nums)));
    ...
    elseif stim_matching_nature == 3
        if stim_or_perf_matched == 1
            % Coherences defined at top of script
            % audInfo.cohSet = [1.0, 0.7, 0.5, 0.3, 0.10, 0.05];
            % visInfo.cohSet = [1.0, 0.7, 0.5, 0.3, 0.10, 0.05];
        elseif stim_or_perf_matched == 2
            % write code for performance matched coherence set
        end

        % Generate matrix based on modalities you want included
        if modality == 1
            all_trials = at_generateMatrixALL_EEG(catchtrials, 0, 0, stimtrials, audInfo, visInfo, right_var, left_var, catch_var, 0);
        elseif modality == 2
            all_trials = at_generateMatrixALL_EEG(catchtrials, 0, 0, stimtrials, audInfo, visInfo, right_var, left_var, catch_var, 0);
        elseif modality == 3 
            all_trials = at_generateMatrixALL_EEG(catchtrials, congruent_mstrials, incongruent_mstrials, stimtrials, audInfo, visInfo, right_var, left_var, catch_var, cued_nature);
        end
...
elseif isCong == 0 && isDir == 1
    ...
    instructions_EEG(curWindow, cWhite0);
end
...
for ii = 1:length(data_output)
   ...
   %% AV Stimuli Presentation
    ...
    [data_output] = record_EEGdata(data_output, right_var, left_var, congruent_keypress, incongruent_keypress, audInfo, visInfo, resp, rt, ii, cong_var, incong_var, markers);
...
% % Counts number of noise trials
% isZeroInFirstFour = data_output(:, 1:4) == 0;
% numRowsWithAllZero = sum(all(isZeroInFirstFour, 2));
% disp(['Number of rows with zeros in ALL of the first 4 columns: ', num2str(numRowsWithAllZero)]);
% 
% % Counts number of visual only trials
% isNanInFirstTwo = isnan(data_output(:, 1:2));              
% numRowsWithBothNan = sum(all(isNanInFirstTwo, 2)); 
% disp(['Number of rows with NaN in BOTH first two columns: ', num2str(numRowsWithBothNan)]);
% 
% % Counts number of incongruent (A left V right) trials
% rowsMatchingCondition = (data_output(:, 1) == 1) & (data_output(:, 3) == 2);
% numMatchingRows = sum(rowsMatchingCondition);
% disp(['Number of rows with 1 in column 1 AND 2 in column 3: ', num2str(numMatchingRows)]);
