5%% HUMAN AUDIOVISUAL MOTION DISCRIMINATION TASK CODE %%%%%%%%%%
...
congruent_keypress = [40 14]; % 1 on button box
incongruent_keypress = [39 15]; % 0 on button box
...
% New conditions:
num_trials = 4; stimtrials = [0, 0, 0]; catchtrials = 0;
congruent_mstrials = 2; incongruent_mstrials = 2;
...
elseif task_nature == 2
    ...
    elseif stim_matching_nature == 3
        ...
        %all_trials = at_generateMatrixALL(catchtrials, congruent_mstrials, incongruent_mstrials, stimtrials, audInfo, visInfo, right_var, left_var, catch_var, cued_nature);
        num_trials_condition = 12;

        congright_trial = [1 1 1 1];
        congright_mat = repmat(congright_trial, num_trials_condition, 1);

        congleft_trial = [2 1 2 1];
        congleft_mat = repmat(congleft_trial, num_trials_condition, 1);

        incongright_trial = [1 1 2 1];
        incongright_mat = repmat(incongright_trial, num_trials_condition, 1);

        incongleft_trial = [2 1 1 1];
        incongleft_mat = repmat(incongleft_trial, num_trials_condition, 1);

        all_trials = [congright_mat; congleft_mat; incongright_mat; incongleft_mat];
        % Randomize trials
         rng('shuffle');
         nbtrials = size(all_trials, 1);
         order = randperm(nbtrials);
...
if training_nature
    instructions_trainCongAV(curWindow, cWhite0, pahandle, corr_soundout, incorr_soundout)
else
...
for ii = 1:length(data_output)
   ...
   %% AV Stimuli Presentation
    ...
    [trial_status, data_output] = record_congAVdata(data_output, right_var, left_var, congruent_keypress, incongruent_keypress, audInfo, visInfo, resp, rt, ii, cong_var, incong_var);
...
%% Feedback for particiapnt on how many trials they got (in)correct
% Calculates score
numCorrect = 0;
for i = 1:size(data_output, 1)
    if data_output(i, 5) == data_output(i, 8)
        numCorrect = numCorrect + 1;
    end
end

% Displays score
Screen('DrawText', curWindow, ['You scored ' num2str(numCorrect) ' out of ' num2str(size(data_output, 1)) ' correct'],500,300,cWhite0);
