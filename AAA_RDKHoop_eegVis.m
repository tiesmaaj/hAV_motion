...
% if EEG_nature == 1
%     addpath('/home/wallace/Human_AV_Motion/liblsl-Matlab-master/');
%     addpath('/home/wallace/Human_AV_Motion/liblsl-Matlab-master/bin/');
...
congruent_keypress = [40 14];
incongruent_keypress = [41 15];
...
% into equal parts to give subject breaks if there ar9e many trials. 
...
cohSet = str2num(cohSet);
modality = input('AO (1), VO (2), or AV (3): ');

if modality == 1
    num_trials = 250; stimtrials = [trials_per_condition, 0, 0]; catchtrials = 25;
elseif modality == 2
        num_trials = 250; stimtrials = [0, trials_per_condition, 0]; catchtrials = 25;
elseif modality == 3 
        num_trials = 250; stimtrials = [trials_per_condition, trials_per_condition, trials_per_condition]; catchtrials = 25;
else
...
elseif task_nature == 2
    ...
    elseif stim_matching_nature == 3
        audInfo.cohSet = cohSet;
        visInfo.cohSet = cohSet;

        all_trials = at_generateMatrixALL(catchtrials, trials_per_condition, trials_per_condition, stimtrials, audInfo, visInfo, right_var, left_var, catch_var, 0);
...
% check display settings on PCstimtrials. curScreen will probably equal 1 or 2 
